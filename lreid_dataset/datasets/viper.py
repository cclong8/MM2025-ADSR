...
class IncrementalSamples4viper(IncrementalPersonReIDSamples):
    ...
    def __init__(self, datasets_root, relabel=True, combineall=False, split_id=0):
        ...
        self.split_path = osp.join(self.dataset_dir, 'file.json')
        self.prepare_split()
        ...
        train = [tuple(item + [20]) for item in train]
        query = [tuple(item + [20]) for item in query]
        gallery = [tuple(item + [20]) for item in gallery]
