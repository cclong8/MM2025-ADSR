...
def make_loss(cfg, num_classes):
    if 'triplet' in cfg.MODEL.METRIC_LOSS_TYPE:
        ...
        raise ValueError(f"expected METRIC_LOSS_TYPE should be triplet but got {cfg.MODEL.METRIC_LOSS_TYPE}")

    if cfg.MODEL.IF_LABELSMOOTH == 'on':
        xent = CrossEntropyLabelSmooth(num_classes=num_classes)
        print("label smooth on, num of classes:", num_classes)
    ...
    if cfg.DATALOADER.SAMPLER == 'softmax':
        ...
        def loss_func(score, feat, target, target_cam):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    ...
                    if isinstance(score, list):
                        ID_LOSS = [F.cross_entropy(scor / (torch.norm(scor, p=2, dim=-1, keepdim=True) + 1e-7) / 0.01, target) for scor in score[1:]]
                        ID_LOSS = sum(ID_LOSS) / len(ID_LOSS)
                        ID_LOSS = 0.5 * ID_LOSS + 0.5 * F.cross_entropy(score[0] / (torch.norm(score[0], p=2, dim=-1, keepdim=True) + 1e-7) / 0.01, target)
                    else:
                        ID_LOSS = F.cross_entropy(score / (torch.norm(score, p=2, dim=-1, keepdim=True) + 1e-7) / 0.01, target)
                    if isinstance(feat, list):
                        TRI_LOSS = [triplet(feats, target)[0] for feats in feat[1:]]
                        TRI_LOSS = sum(TRI_LOSS) / len(TRI_LOSS)
                        TRI_LOSS = 0.5 * TRI_LOSS + 0.5 * triplet(feat[0], target)[0]
        ...
        raise ValueError(f'expected sampler should be softmax, triplet, softmax_triplet or softmax_triplet_center but got {cfg.DATALOADER.SAMPLER}')
