...
def extract_features_voro(model, data_loader, get_mean_feature=False):
    ...
    if get_mean_feature:
        ...
        labels_named = list(set(labels_all))  # obtain valid features
        labels_named.sort()
        features_mean = [] # 类别平均特征向量
        for x in labels_named:
            if x in features_collect.keys():
                features_mean.append(torch.stack(features_collect[x]).mean(dim=0)) # 计算每个标签对应特征的均值，作为该类的“中心”特征向量。
            else:
                features_mean.append(torch.zeros_like(features_all[0])) # 如果某标签没对应特征（理论上不会），用零向量代替。
