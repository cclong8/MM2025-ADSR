...
class Backbone(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, block, num_classes, layers):
        ...
        self.in_planes = 2048  # 表示网络最后一层特征通道数为2048
        self.base = ResNet(last_stride=last_stride, block=block, layers=layers)
        print('using resnet50 as a backbone')

        self.bottleneck = nn.BatchNorm2d(2048)  # 定义一个二维批归一化层（BatchNorm），作用于2048通道的特征图，常用于稳定训练
        self.bottleneck.bias.requires_grad_(False)  # 关闭批归一化层偏置的梯度更新，即不训练bias参数。
        nn.init.constant_(self.bottleneck.weight, 1)  # 初始化BatchNorm的权重为1，偏置为0，保持初始归一化功能。
        nn.init.constant_(self.bottleneck.bias, 0)  # 初始化BatchNorm的权重为1，偏置为0，保持初始归一化功能。

        self.pooling_layer = GeneralizedMeanPoolingP(3)  # 定义一个自定义的池化层GeneralizedMeanPoolingP，参数为3，常用来替代传统的平均池化或最大池化，获得更灵活的特征聚合。
    ...
    def forward_head(self, x):  # 单独执行分类头部分的前向传递过程
    ...
    def forward(self, x, domains=None, training_phase=None, get_all_feat=False, epoch=0, head_only=False):
        ...
        global_feat = self.pooling_layer(x)  # [B, 2048, 1, 1]
        bn_feat = self.bottleneck(global_feat)  # [B, 2048, 1, 1]
        # global_feat = F.normalize(global_feat)  # L2 normalization，默认dim=1

        if get_all_feat is True:  # 跳过训练模式判断，用于计算旧模型的特征
            cls_outputs = self.classifier(bn_feat[..., 0, 0])
            return global_feat[..., 0, 0], bn_feat[..., 0, 0], cls_outputs, x  # 返回全局池化特征，bn特征，分类结果和骨干网络输出。

        if self.training is False:
            # return bn_feat[..., 0, 0]
            return global_feat[..., 0, 0]  # 如果模型处于评估模式（非训练），只返回全局池化特征，方便做检索或测试。

        bn_feat = bn_feat[..., 0, 0]
        cls_outputs = self.classifier(bn_feat)
        return global_feat[..., 0, 0], bn_feat, cls_outputs, x  # 训练模式下返回全局池化特征，bn特征，分类结果和骨干网络输出。
